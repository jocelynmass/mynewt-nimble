...
#define nrf5340_hal_timer_MAX   (5)
...
static void
hal_rtc_timer_irq_handler(struct nrf5340_hal_timer *bsptimer)
{
    uint32_t overflow;
    uint32_t compare;
    uint32_t tick;
...
#if MYNEWT_VAL(TIMER_1)
void
nrf5340_timer1_irq_handler(void)
{
    hal_timer_irq_handler(&nrf5340_hal_timer1);
}
#endif

#if MYNEWT_VAL(TIMER_2)
void
nrf5340_timer2_irq_handler(void)
{
    hal_timer_irq_handler(&nrf5340_hal_timer2);
}
#endif

#if MYNEWT_VAL(TIMER_3)
void
nrf5340_timer3_irq_handler(void)
{
    hal_rtc_timer_irq_handler(&nrf5340_hal_timer3);
}
#endif

#if MYNEWT_VAL(TIMER_4)
void
nrf5340_timer4_irq_handler(void)
{
    hal_rtc_timer_irq_handler(&nrf5340_hal_timer4);
}
#endif
...
hal_timer_stop(struct hal_timer *timer)
{
    ...
    if (timer->link.tqe_prev != NULL) {
        ...
        if (reset_ocmp) {
            if (entry) {
                nrf_timer_set_ocmp((struct nrf5340_hal_timer *)entry->bsp_timer,
                                   entry->expiry);
            } else {
                if (bsptimer->tmr_rtc) {
                    nrf_rtc_disable_ocmp((NRF_RTC_Type *)bsptimer->tmr_reg);
